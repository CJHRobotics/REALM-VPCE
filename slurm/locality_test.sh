#!/bin/bash
#
# Channel locality test on GAIVI.
#
# Answers whether the hog channel carries no location information, or whether
# our width statistic fails in 30,240 dimensions. Measures the feature-space
# width of a disc of floor (an upper bound on how localised any grouping could
# be), the floor distance to the most similar views, and the concentration of
# pairwise distances. Uses no clustering, no rules and no thresholds.
#
# Usage:
#   sbatch slurm/locality_test.sh [env_name] [extra args...]
#
# Examples:
#   sbatch slurm/locality_test.sh circ_lm8_r0
#   sbatch slurm/locality_test.sh circ_lm8_r0 --radii 0.5,1.0,2.0
#   sbatch --gres=gpu:1 slurm/locality_test.sh circ_lm8_r0   # any free GPU
#
# ------------------------------------------------------------- SLURM header
#SBATCH --job-name=locality
...
set +e
python analysis/experiment_channel_isolation/run_locality_test.py \
    "${ENV}" "${EXTRA_ARGS[@]}"
STATUS=$?
set -e
...
if [[ ${STATUS} -ne 0 && -n "${EMAIL_TO:-}" ]]; then
    python - "${ENV}" "${JOB_ID}" "${STATUS}" \
             "slurm/logs/${SLURM_JOB_NAME:-locality}-${JOB_ID}.out" <<'PY' \
...
send_email(f'[REALM-VPCE] {env} locality-test FAILED (exit {status}, job {job})',
